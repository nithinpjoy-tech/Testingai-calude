"""
p01_dashboard.py — Upload, KPI summary, run history table.
Features: #1 Upload, #2 History, #3 Status cards.
"""
...
def render() -> None:
    ...
    st.markdown(
        "<p style='color:#6B7885;font-size:0.88rem;'>"
        "Drop a JSON, XML or LOG file exported from your test harness.</p>",
        unsafe_allow_html=True,
    )

    uploaded = st.file_uploader(
        "Select file", type=["json", "xml", "log", "txt"],
    ...
    if uploaded:
        ...
        col_info.markdown(
            f"<div class='tt-card tt-card-accent'>"
            f"<b>{uploaded.name}</b><br>"
            f"<span style='color:#6B7885;font-size:0.82rem;'>"
            f"{uploaded.size:,} bytes · {uploaded.type or 'unknown type'}</span></div>",
            unsafe_allow_html=True,
        )
    ...
    s3.button("📄 LOG sample", use_container_width=True, disabled=True,
              help="Raw log ingestion coming in Step 6")
